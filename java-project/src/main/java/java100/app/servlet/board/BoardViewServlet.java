    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (board != null) {
                ...
                out.println("<div class='col-sm-10'>");
                out.printf("<input class='form-control' readonly id='regdt' type='text' name='regdt' value='%s'>\n",
                        board.getRegDate());
                out.println("</div>");
                out.println("</div>");
                out.println("<div class='form-group row'>");
                ...
                out.printf("<a href='delete?no=%d' class='btn btn-primary btn-sm'>삭제</a>", board.getNo());
                out.println("</form>");
            } else {
                out.printf("<p>%s의 게시물 정보가 없습니다.</p>", no);
            }
        } catch (Exception e) {
            e.printStackTrace();
