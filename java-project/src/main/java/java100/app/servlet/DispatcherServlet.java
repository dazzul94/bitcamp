...
public class DispatcherServlet extends HttpServlet {
        ...
        for (int i = 0; i < params.length; i++) {
            if (params[i].getType() == HttpServletRequest.class) {
                ...
                if (anno != null) {
                    ...
                    values[i] = createParamObject(params[i], request);
                }
    ...
    private Object createParamObject(Parameter parameter, HttpServletRequest request) {
        Class<?> clazz = parameter.getType();
        try {
            Object obj = clazz.newInstance();
            @SuppressWarnings("unchecked")
            Set<Method> setters = getMethods(clazz, withPrefix("set"), withParametersCount(1));
            for (Method setter : setters) { //셋터 메서드 호출
