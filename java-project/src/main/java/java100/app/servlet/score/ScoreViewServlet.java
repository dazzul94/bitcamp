    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (score != null) {
                ...
                out.println("<div class='col-sm-10'>");
                out.printf("<input class='form-control' readonly id='aver' type='text' name='aver' value='%.1f'>\n",
                        score.getAver());
                out.println("</div>");
                out.println("</div>");
                out.println("<div class='form-group row'>");
                ...
                out.printf("<a href='delete?no=%d' class='btn btn btn-danger btn-sm'>삭제</a>", score.getNo());
                out.println("</div>");
                out.println("</div>");
                out.println("</form>");

            } else {
                out.printf("<p>%s의 성적 정보가 없습니다.</p>", no);
            }
        } catch (Exception e) {
            e.printStackTrace();
