...
&lt;%@
 ...
 JSP 페이지를 작성할 때 사용한 문자 집합을 지정한다.<br>
 JSP 엔진이 템플릿 데이터를 출력문으로 바꿀 때 참조한다.<br>
 JSP 파일을 'UTF-8'로 젖아했으면, 이 속성의 값도 'UTF-8'로 지정하라!<br>
 생략해도 된다.<br>
 
<h3>trimDirectiveWhitespaces 속성</h3>
<p>지시부 태그들 사이에 삽입된 공백(스페이스, 줄바꿈, 탭 등)을 출력문으로 바꾸지 않는다.<br>
</p>

<h3>autoFlush 속성</h3>
<p>
출력한 콘텐츠가 버퍼에 꽉찼을 때 자동으로 출력할지를 결정한다.<br>
true이면, 자동으로 출력한다. <b>true가 기본값이다.</b><br>
false로 설정하면, 버퍼가 찼을 때 실행오류를 발생시킨다.<br>
즉 false로 설정했을때는 개발자가 버퍼가 차기 전에 
...
이를 바꾸고 싶다면 이속성에 다른 페이지의 URL을 입력하라!<br>
...
//import="java.lang.*"
import java.lang.*;
//import="java.lang.String, java.utl.*, java.net.*"
import java.lang.String;
import java.util.*;
import java.net.*;

//다음과 같이 page 태그를 여러개 추가해도 된다.
//
